...
public class OperationSystemClient {
    private static final int CPUTIME = 500;
    private static final int PERCENT = 100;
    private static final int FAULTLENGTH = 10;
    private static OperationSystemClient osc = new OperationSystemClient();
    private static String osName = System.getProperty("os.name");
    private Logger logger = Logger.getLogger(this.getClass());

    public OperationSystemClient() {
    ...
    public static void main(String[] args) throws Exception {
        ...
        OperationSystemClient osc = new OperationSystemClient();
        double cpuUsage = osc.getCpuUsage();
        // 当前系统的内存使用率
        double memUsage = osc.getMemUsage();
        // 当前系统的硬盘使用率
        double diskUsage = osc.getDiskUsage();
        ...
        if (osName.toLowerCase().contains("windows")
            ...
            try {
                String procCmd = System.getenv("windir")
                        + "//system32//wbem//wmic.exe MyProcess get Caption,CommandLine,KernelModeTime,ReadOperationCount,ThreadCount,UserModeTime,WriteOperationCount";
                // 取进程信息
            ...
            try {
                Map<?, ?> map1 = osc.cpuinfo();
                Thread.sleep(CPUTIME);
                Map<?, ?> map2 = osc.cpuinfo();
        ...
        if (osName.toLowerCase().contains("windows")
            ...
            try {
                OperatingSystemMXBean osmxb = (OperatingSystemMXBean) ManagementFactory
                        .getOperatingSystemMXBean(); // 总的物理内存+虚拟内存 long
                long totalvirtualMemory = osmxb.getTotalSwapSpaceSize(); // 剩余的物理内存
                // long
                double freePhysicalMemorySize = osmxb
